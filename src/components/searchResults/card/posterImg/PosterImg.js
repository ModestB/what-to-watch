...
const posterImg = (props) => {
  ...
  let content = 
    <div 
      className={`${ [classes.posterImg, classes.posterImgPlaceholder, posterImgSize].join(' ')}`}>
        <NoImage fill="#ffffff" width="50px" height="50px" />
    </div>;

  if(props.posterPath) {
    content = <img className={`${ [classes.posterImg, posterImgSize ].join(' ')}`} src={ imgSrc + props.posterPath } />;
  }
