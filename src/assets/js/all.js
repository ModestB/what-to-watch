...
export function searchSuggestionSelectHandler() {
  ...
  suggestionsContainer.addEventListener('mouseover', () => {
    readySuggestion = document.querySelector('.suggestions-container .suggestion.ready');
    if (readySuggestion) {
  ...
  input.addEventListener('keydown', (e) => {
    suggestions = Array.from(document.querySelectorAll('.suggestions-container .suggestion'));
    readySuggestion = document.querySelector('.suggestions-container .suggestion.ready');
    // Arrow Down Press
    if (e.keyCode === 40 && suggestions.length > 0) {
      if (readySuggestion) {
        moveSuggestionHandler(readySuggestion, suggestionsContainer, suggestions, true);
      } else {
    ...
    // Arrow Up Press  
      ...
      if (readySuggestion) {
        moveSuggestionHandler(readySuggestion, suggestionsContainer, suggestions, false);
      } else {
        return false;
      };
    };
...
function moveSuggestionHandler(readySuggestion, suggestionsContainer, suggestions, moveDown) {
  ...
  suggestionsContainer.scrollTop = moveDown ? scrollTopPossition + nextEleHeight : scrollTopPossition - nextEleHeight;
};
